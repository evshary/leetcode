...
public:
    vector<int> largestDivisibleSubset(vector<int>& nums) {
        ...
        vector<int> dp(copy_nums.size(), 1);
        vector<int> track(copy_nums.size(), -1);
        int max_len = 0, track_ptr;
        for (int i = 0; i < copy_nums.size(); i++) {
            for (int j = 0; j < i; j++) {
                if (copy_nums[i] % copy_nums[j] == 0 && dp[i] < dp[j]+1) {
                    dp[i] = dp[j] + 1;
                    track[i] = j;
                }
            }
        ...
        vector<int> ret;
        while(track_ptr != -1) {
            ret.insert(ret.begin(), copy_nums[track_ptr]);
            track_ptr = track[track_ptr];
        }
        return ret;
    }
};
...
public:
    vector<int> largestDivisibleSubset(vector<int>& nums) {
        vector<int> copy_nums(nums);
        sort(copy_nums.begin(), copy_nums.end());
        return largestDivisibleSubset_r(vector<int>(), 0, copy_nums);
    }
private:
    vector<int> largestDivisibleSubset_r(vector<int> tmp_v, int cur_idx, vector<int>& nums) {
        vector<int> take, not_take;
        if (cur_idx == nums.size())
            return tmp_v;
        not_take = largestDivisibleSubset_r(tmp_v, cur_idx+1, nums);
        if (tmp_v.empty() || (nums[cur_idx] % tmp_v.back()) == 0) {
            tmp_v.push_back(nums[cur_idx]);
            take = largestDivisibleSubset_r(tmp_v, cur_idx+1, nums);
        }
...
int main() {
    vector<vector<int>> problems{{1,2,3}, 
                                 {1,2,4,8},
